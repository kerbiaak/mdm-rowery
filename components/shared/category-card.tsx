...
export function CategoryCard({ category }: { category: Category }) {
  const { title, desc, img, fit, icon: Icon, href } = category;
  return (
    <Link
      href={href}
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-white">
        ...
        />
        <span className="absolute left-5 top-5 flex size-11 items-center justify-center rounded-2xl border border-black/[0.05] bg-white/90 text-ink shadow-sm backdrop-blur-sm transition-colors duration-300 group-hover:border-brand group-hover:bg-brand group-hover:text-white">
